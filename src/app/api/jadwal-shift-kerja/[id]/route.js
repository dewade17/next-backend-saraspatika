...
import { getTodayJadwalShiftKerjaService } from '@/services/shift_kerja/jadwal_shift_kerja_service.js';

export const runtime = 'nodejs';

// Fungsi requirePerm yang sudah mendukung Header Authorization & Cookie
async function requirePerm(req, resource, action) {
  const authHeader = req.headers.get('Authorization');
  let token = null;

  // 1. Coba ambil token dari Header Authorization (Bearer)
  if (authHeader && authHeader.startsWith('Bearer ')) {
    token = authHeader.substring(7);
  }

  // 2. Jika di header tidak ada, coba ambil dari Cookie
  if (!token) {
...
export const GET = apiRoute(async (req, context) => {
  // Sekarang kita mengirimkan 'req' ke requirePerm
  await requirePerm(req, 'shift_kerja', 'read');

  const params = await context.params;
  const id_user = params.id;
  const data = await getTodayJadwalShiftKerjaService({ id_user });
