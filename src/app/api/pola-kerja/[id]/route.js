...
import { polaKerjaUpdateValidation } from '@/validations/pola_kerja/pola_kerja_validation.js';
import { deletePolaKerjaService, getPolaKerjaByIdService, updatePolaKerjaService } from '@/services/pola_kerja/pola_kerja_service.js';
...
export const GET = apiRoute(async (req, ctx) => {
  await requirePerm(req, 'pola_jam_kerja', 'read');

  const params = await ctx.params;
  const id = params?.id;

  const data = await getPolaKerjaByIdService(id);
...
export const PATCH = apiRoute(async (req, ctx) => {
  await requirePerm(req, 'pola_jam_kerja', 'update');

  const params = await ctx.params;
  const id = params?.id;

  const input = await parseBody(req, polaKerjaUpdateValidation);
  const data = await updatePolaKerjaService(id, input);
...
export const DELETE = apiRoute(async (req, ctx) => {
  await requirePerm(req, 'pola_jam_kerja', 'delete');

  const params = await ctx.params;
  const id = params?.id;

  const data = await deletePolaKerjaService(id);
